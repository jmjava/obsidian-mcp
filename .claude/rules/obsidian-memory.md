...
When claiming, blocking, unblocking, or finishing a Next Steps or Agent Queue item, prefer `list_open_tasks`, `list_blocked_tasks`, `claim_task`, `block_task`, `unblock_task`, and `complete_task` over rewriting the whole state by hand. Those tools preserve other Project State sections, may check a matching Agent Queue checkbox (`claim_task` / `complete_task` only), and do not overwrite daily notes.
